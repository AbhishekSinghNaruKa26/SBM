        ...
        <div className='flex gap-7 mt-6 overflow-x-auto w-full mx-auto'>
            {exclusiveProducts.map((product  , index)=>(
                <div key={index} className='px-3 border border-black min-w-48 max-w-48 whitespace-nowrap truncate rounded-lg mx-3 '>
                    <div onClick={()=>navigate('/products/cart')}  className='flex justify-center my-2'><img className='h-30 w-20' src={product.img} alt="" /></div> 

                    <div className='font-bold my-1 text-red-600'><h1>{product.companyName}</h1></div>

                    <div onClick={()=>navigate('/products/cart')}  className='font-semibold my-1'>{product.productName}</div>
                    ...
                    {/* Add TO CArt Button */}
                    <div onClick={()=>navigate('/AddToCart')} className=' my-2 rounded-lg text-white flex justify-center'>

                      <button className='group relative bg-black rounded-lg py-1 w-full '>
